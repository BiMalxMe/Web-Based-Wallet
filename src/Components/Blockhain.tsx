...
export const Blockchain = ({ type }: BlockProps) => {
  ...
  const [passwords, setPasswords] = useState<{ [key: number]: boolean }>({}); // Individual password visibility track garne  
      ...
      {shouldDisplayMnemonic && (
        <div>
          <div className="flex justify-between items-center gap-4 bg-slate-900 rounded-2xl w-full px-4 py-2">
            ...
            <div
              className="hover:bg-slate-700 rounded-full p-2 cursor-pointer"
          ...
          {!extend && (
            <ul className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 m-4">
              {globalMnemonicWords.map((word, index) => (
                <li
                  key={index}
                  className="px-6 py-4 bg-slate-900 rounded-xl text-center text-sm sm:text-base"
                >
                  {word}
                </li>
              ))}
      ...
      <div>
        ...
        {wallets.length === 0 ? (
          <div className="w-full">
            <p className="text-center text-sm sm:text-base bg-gradient-to-r from-blue-400 to-teal-300 bg-clip-text text-transparent font-medium py-2">
          ...
          wallets.map((wallet, index) => (
            <div
              key={index}
              className="relative border border-slate-900 rounded-2xl p-3 md:p-4 shadow-lg hover:bg-slate-700 transition-all my-3 md:my-4 flex flex-col w-full"
            >
              <p className="text-center font-extrabold bg-slate-600 py-1 px-2 rounded-t-lg text-sm md:text-base w-full">
                Wallet {index + 1}
              </p>
                ...
                <div className="w-full">
                  ...
                  <div className="my-2 md:my-4 w-full">
                    ...
                    <div className="flex items-center w-full">
                      <input
                        type={passwords[index] ? "text" : "password"}
                      ...
                      />
                      <button
                        onClick={() => setPasswords((prev) => ({ ...prev, [index]: !prev[index] }))}
                        className="ml-2 p-1"
                      >
                        {passwords[index] ? <Show /> : <Hide />}
                      </button>
                    </div>
                  </div>
                </div>
